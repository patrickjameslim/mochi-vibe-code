...
function GroupCombobox({
  ...
  // Groups that match query but aren't in the master list (user typed a new one)
  const canCreate =
    query.trim() &&
    !ALL_GROUPS.some((g) => g.toLowerCase() === query.trim().toLowerCase()) &&
    !selected.some((g) => g.toLowerCase() === query.trim().toLowerCase());

  function createGroup() {
    const name = query.trim();
    if (!name) return;
    ALL_GROUPS.push(name); // add to master list for this session
